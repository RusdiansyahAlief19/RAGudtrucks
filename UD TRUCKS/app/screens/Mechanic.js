    ...
    <div style={{ display: "flex", gap: 12, maxWidth: "82%" }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px 14px 14px 14px",
          ...
          <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.65, whiteSpace: "pre-wrap" }}>
    ...
    try {
      ...
      if (data.error) {
        ...
        setMsgs((m) => [...m, { 
            ...
            found: uniqueSources.length
        }]);
      }
    } catch (err) {
      setTyping(false);
      setMsgs((m) => [...m, { role: "ai", text: "**Koneksi Gagal**: Tidak dapat terhubung ke server Uvicorn (127.0.0.1:8000).", warn: true }]);
    }
  };
